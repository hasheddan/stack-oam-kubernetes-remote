...
// SetupContainerizedWorkload adds a controller that reconciles ContainerizedWorkloads remotely.
func SetupContainerizedWorkload(mgr ctrl.Manager, l logging.Logger) error {
	name := "remote.oam/" + strings.ToLower(oamv1alpha2.ContainerizedWorkloadGroupKind)
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		Complete(workload.NewReconciler(mgr,
			...
			workload.WithPacker(workload.NewPackagerWithWrappers(client.Packager, workload.KubeAppWrapper)),
		))
}
